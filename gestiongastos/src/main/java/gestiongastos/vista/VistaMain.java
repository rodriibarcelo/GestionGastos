...
public class VistaMain {
    ...
    public VistaMain() {
        ...
        // NUEVO: Menú Ver (Estadísticas y Calendario)
        Menu menuVer = new Menu("Ver");
        MenuItem miEstadisticas = new MenuItem("Estadísticas…");
        MenuItem miCalendario  = new MenuItem("Calendario (CalendarFX)...");
        ...
        // Acciones a la derecha (Filtrar, Añadir, Editar, Eliminar)
        ...
        table.setRowFactory(tv -> {
            ...
            row2.setOnMouseClicked(evt -> {
                if (evt.getClickCount() == 2 && !row2.isEmpty()) onEdit();
            });
            return row2;
        });
      ...
      /*  // NUEVO: abrir calendario (si no está CalendarFX, muestra aviso)
        miCalendario.setOnAction(e -> {
            try {
                VistaCalendarioGastos cal = new VistaCalendarioGastos();
                Utils.openDialog(root, "Gastos en calendario", cal.getRoot(), 1024, 720);
            } catch (Throwable t) {
                Utils.alertWarn("No se ha podido abrir el calendario.\n" +
                        "Añade CalendarFX al proyecto o revisa dependencias.\n\n" + t.getClass().getSimpleName() + ": " + t.getMessage());
            }
        });*/
    ...
    private void onEdit() {
        ...
        cb.getItems().stream().filter(c -> c.getNombre().equals(sel.getCategoria())).findFirst().ifPresent(c -> cb.getSelectionModel().select(c));

        TextField tfN = new TextField(sel.getNota());

        GridPane gp = new GridPane();
        gp.setHgap(10); gp.setVgap(10); gp.setPadding(new Insets(10));
        gp.add(new Label("Fecha:"), 0, 0); gp.add(dp,   1, 0);
        gp.add(new Label("Cantidad (€):"),0, 1); gp.add(tfImp,1, 1);
        gp.add(new Label("Categoría:"), 0, 2); gp.add(cb,   1, 2);
        gp.add(new Label("Nota:"),      0, 3); gp.add(tfN,  1, 3);
        ...
        try {
            ...
            if (nuevaFecha == null) throw new IllegalArgumentException("Selecciona una fecha.");
            if (nuevaCat == null) throw new IllegalArgumentException("Selecciona una categoría.");
            if (nuevaCantidad.compareTo(BigDecimal.ZERO) <= 0) throw new IllegalArgumentException("La cantidad debe ser mayor que 0.");
    ...
    private static BigDecimal parseCantidad(String txt) {
        if (txt.isBlank()) throw new IllegalArgumentException("Introduce una cantidad.");
        txt = txt.replace(',', '.');
    ...
    public static class GastoRow {
        ...
        public Gasto getOriginal() { return original; }
